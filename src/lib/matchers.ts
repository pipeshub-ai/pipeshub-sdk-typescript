...
import { PipeshubDefaultError } from "../models/errors/pipeshub-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | PipeshubDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | PipeshubDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new PipeshubDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new PipeshubDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
