...
const [, convId, oldTitle] = decodeComplete(completeData);

console.log(`conversation id: ${convId}`);

const titleRes = await pipeshub.agents.updateAgentConversationTitle({
  agentKey: key,
  conversationId: convId,
  body: { title: NEW_TITLE },
});

const newTitle = titleRes.conversation?.title;
if (newTitle == null) {
