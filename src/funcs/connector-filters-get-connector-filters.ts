...
export function connectorFiltersGetConnectorFilters(
  client: PipeshubCore,
  security: operations.GetConnectorFiltersSecurity,
  request: operations.GetConnectorFiltersRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetConnectorFiltersResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetConnectorFiltersRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetConnectorFiltersResponse$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
