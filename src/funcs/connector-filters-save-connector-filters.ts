...
export function connectorFiltersSaveConnectorFilters(
  ...
  Result<
    operations.SaveConnectorFiltersResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.SaveConnectorFiltersRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.SaveConnectorFiltersResponse$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
