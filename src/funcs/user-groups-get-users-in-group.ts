...
export function userGroupsGetUsersInGroup(
  ...
  Result<
    operations.GetUsersInGroupResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetUsersInGroupRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/userGroups/{groupId}/users")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetUsersInGroupResponse$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
