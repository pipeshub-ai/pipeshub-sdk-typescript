...
export function documentManagementCreateDocumentBufferMultipart(
  ...
  Result<
    operations.CreateDocumentBufferMultipartResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.CreateDocumentBufferMultipartRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CreateDocumentBufferMultipartResponse$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
