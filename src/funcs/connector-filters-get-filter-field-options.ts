...
export function connectorFiltersGetFilterFieldOptions(
  client: PipeshubCore,
  security: operations.GetFilterFieldOptionsSecurity,
  request: operations.GetFilterFieldOptionsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetFilterFieldOptionsResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetFilterFieldOptionsRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetFilterFieldOptionsResponse$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
