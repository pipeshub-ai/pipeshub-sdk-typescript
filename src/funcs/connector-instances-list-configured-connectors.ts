...
export function connectorInstancesListConfiguredConnectors(
  ...
  Result<
    operations.ListConfiguredConnectorsResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.ListConfiguredConnectorsRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.ListConfiguredConnectorsResponse$inboundSchema),
    M.fail([401, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
