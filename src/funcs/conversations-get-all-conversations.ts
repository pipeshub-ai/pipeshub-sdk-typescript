...
export function conversationsGetAllConversations(
  ...
  Result<
    operations.GetAllConversationsResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.GetAllConversationsRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetAllConversationsResponse$inboundSchema),
    M.fail([400, 401, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
