...
export function conversationsGetArchivedConversations(
  ...
  Result<
    operations.GetArchivedConversationsResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.GetArchivedConversationsRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetArchivedConversationsResponse$inboundSchema),
    M.fail([401, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
