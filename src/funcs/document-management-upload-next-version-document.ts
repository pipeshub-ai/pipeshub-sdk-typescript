...
export function documentManagementUploadNextVersionDocument(
  ...
  Result<
    operations.UploadNextVersionDocumentResponse,
    | PipeshubError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.UploadNextVersionDocumentRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.UploadNextVersionDocumentResponse$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
