/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { agentsCreateAgent } from "../funcs/agents-create-agent.js";
import { agentsDeleteAgent } from "../funcs/agents-delete-agent.js";
import { agentsGetAgentPermissions } from "../funcs/agents-get-agent-permissions.js";
import { agentsGetAgent } from "../funcs/agents-get-agent.js";
import { agentsListAgentTools } from "../funcs/agents-list-agent-tools.js";
import { agentsListAgents } from "../funcs/agents-list-agents.js";
import { agentsShareAgent } from "../funcs/agents-share-agent.js";
import { agentsUpdateAgentPermissions } from "../funcs/agents-update-agent-permissions.js";
import { agentsUpdateAgent } from "../funcs/agents-update-agent.js";
import { ClientSDK, RequestOptions } from "../lib/sdks.js";
import * as models from "../models/index.js";
import * as operations from "../models/operations/index.js";
import { unwrapAsync } from "../types/fp.js";

export class Agents extends ClientSDK {
  /**
   * List agents
   *
   * @remarks
   * Retrieve all agents available to the authenticated user.<br><br>
   * <b>Overview:</b><br>
   * Returns agents created by the user plus public agents in the organization.
   * Each agent has unique capabilities defined by its tools and knowledge scope.
   */
  async listAgents(
    security: operations.ListAgentsSecurity,
    options?: RequestOptions,
  ): Promise<Array<models.Agent>> {
    return unwrapAsync(agentsListAgents(
      this,
      security,
      options,
    ));
  }

  /**
   * Create agent
   *
   * @remarks
   * Create a new custom AI agent.<br><br>
   * <b>Overview:</b><br>
   * Agents are specialized AI assistants configured for specific tasks.
   * They can have custom system prompts, access to specific tools, and
   * be limited to certain knowledge bases.<br><br>
   * <b>Agent Configuration:</b><br>
   * <ul>
   * <li><b>System prompt:</b> Instructions that define agent behavior</li>
   * <li><b>Tools:</b> Capabilities like web search, code execution, etc.</li>
   * <li><b>Knowledge bases:</b> Data sources the agent can access</li>
   * <li><b>Model config:</b> AI model settings (temperature, max tokens)</li>
   * </ul>
   * <b>Use Cases:</b><br>
   * <ul>
   * <li>Customer support bot with product knowledge</li>
   * <li>Code review assistant with repository access</li>
   * <li>HR assistant with policy documents</li>
   * </ul>
   */
  async createAgent(
    security: operations.CreateAgentSecurity,
    request: operations.CreateAgentRequest,
    options?: RequestOptions,
  ): Promise<models.Agent> {
    return unwrapAsync(agentsCreateAgent(
      this,
      security,
      request,
      options,
    ));
  }

  /**
   * List available tools
   *
   * @remarks
   * Get all tools that can be assigned to agents.<br><br>
   * <b>Overview:</b><br>
   * Tools extend agent capabilities beyond basic Q&A. Each tool
   * has specific inputs and outputs defined by its schema.<br><br>
   * <b>Common Tools:</b><br>
   * <ul>
   * <li><b>web-search:</b> Search the internet</li>
   * <li><b>code-interpreter:</b> Execute code snippets</li>
   * <li><b>file-reader:</b> Read uploaded files</li>
   * <li><b>api-caller:</b> Make external API requests</li>
   * </ul>
   */
  async listAgentTools(
    security: operations.ListAgentToolsSecurity,
    options?: RequestOptions,
  ): Promise<Array<models.AgentTool>> {
    return unwrapAsync(agentsListAgentTools(
      this,
      security,
      options,
    ));
  }

  /**
   * Get agent
   *
   * @remarks
   * Retrieve agent details by its unique key.
   */
  async getAgent(
    security: operations.GetAgentSecurity,
    request: operations.GetAgentRequest,
    options?: RequestOptions,
  ): Promise<models.Agent> {
    return unwrapAsync(agentsGetAgent(
      this,
      security,
      request,
      options,
    ));
  }

  /**
   * Update agent
   *
   * @remarks
   * Update an existing agent's configuration.<br><br>
   * <b>Permissions:</b><br>
   * Only the agent creator can update it.
   */
  async updateAgent(
    security: operations.UpdateAgentSecurity,
    request: operations.UpdateAgentRequest,
    options?: RequestOptions,
  ): Promise<models.Agent> {
    return unwrapAsync(agentsUpdateAgent(
      this,
      security,
      request,
      options,
    ));
  }

  /**
   * Delete agent
   *
   * @remarks
   * Delete an agent.<br><br>
   * <b>Warning:</b><br>
   * All conversations with this agent will become inaccessible.
   */
  async deleteAgent(
    security: operations.DeleteAgentSecurity,
    request: operations.DeleteAgentRequest,
    options?: RequestOptions,
  ): Promise<void> {
    return unwrapAsync(agentsDeleteAgent(
      this,
      security,
      request,
      options,
    ));
  }

  /**
   * Get agent permissions
   *
   * @remarks
   * Get the current permission configuration for an agent.
   */
  async getAgentPermissions(
    security: operations.GetAgentPermissionsSecurity,
    request: operations.GetAgentPermissionsRequest,
    options?: RequestOptions,
  ): Promise<operations.GetAgentPermissionsResponse> {
    return unwrapAsync(agentsGetAgentPermissions(
      this,
      security,
      request,
      options,
    ));
  }

  /**
   * Update agent permissions
   *
   * @remarks
   * Update who can access and use the agent.
   */
  async updateAgentPermissions(
    security: operations.UpdateAgentPermissionsSecurity,
    request: operations.UpdateAgentPermissionsRequest,
    options?: RequestOptions,
  ): Promise<void> {
    return unwrapAsync(agentsUpdateAgentPermissions(
      this,
      security,
      request,
      options,
    ));
  }

  /**
   * Share agent
   *
   * @remarks
   * Share an agent with specific users.
   */
  async shareAgent(
    security: operations.ShareAgentSecurity,
    request: operations.ShareAgentRequest,
    options?: RequestOptions,
  ): Promise<models.Agent> {
    return unwrapAsync(agentsShareAgent(
      this,
      security,
      request,
      options,
    ));
  }
}
